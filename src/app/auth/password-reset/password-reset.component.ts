...
export class PasswordResetComponent {
  ...
  constructor(
    ...
    this.requestUrl = host.startsWith('https')
      ? `${host}${environment.baseUrl}validate-request/`
      : `${host}${environment.baseUrlIntranet}validate-request/`;
  }
  ...
  onSubmit() {
    if (!this.user) {
      this.messageService.add({
        ...
        summary: 'Error',
        detail:
          'Este campo es obligatorio. Por favor, complétalo para continuar',
      });
      return;
    }
      ...
      (response1) => {
        if (response1) {
          ...
          this.authService.requestPasswordReset(this.user).subscribe(
            (response) => {
              ...
              if (response === 'Ya existe una solicitud pendiente') {
                ...
                this.alertType = 'info';
                this.alertMessage =
                  'Por favor, revisa tu buzón de correo electrónico para obtener el enlace de recuperación. Si no encuentras el correo en tu bandeja de entrada, asegúrate de revisar también tu carpeta de correo no deseado o contáctanos al correo: oficinavirtual@activos.com.co';
                this.showAlert = true;

                setTimeout(() => {
                  this.showAlert = false;
                }, 7000); // Mostrar la alerta durante 15 segundos
                ...
                this.authService
                  .sendResetEmail(
                    this.mail_envio,
                    this.user,
                    this.requestUrl + this.RCS_ID + '/BMX' + '/860090915',
                  )
                  .subscribe(
                    (resp) => {
                      if (resp) {
            ...
            (error) => {
              this.alertType = 'info';
              this.alertMessage2 =
                'Por favor, revisa tu buzón de correo electrónico para obtener el enlace <br> de recuperación. Si no encuentras el correo en tu bandeja de entrada,<br> asegúrate de revisar también tu carpeta de correo no deseado o<br> contáctanos al correo: <strong>oficinavirtual@activos.com.co</strong>';
          ...
          this.messageService.add({
            ...
            summary: 'Error',
            detail:
              'El nombre de usuario ingresado es incorrecto o no se encuentra registrado. Asegúrate de haberlo escrito correctamente y de que tu cuenta esté activa.',
          });
        }
      },
      (error) => {
